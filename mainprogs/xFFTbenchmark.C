...
void randomInput() {
/*  for (int I=0; I<8*L0*L1*L2*L3; I++) {
    input[I].x = NaN;
    input[I].y = NaN;    
  }*/
  for (int I=0; I<8*16*16*16*32; I++) {
...
long int calcTheoreticalFlops() {
  long int cycles = (long int) ((log(L0*L1*L2*L3)/log(2)) * 8*L0*L1*L2*L3*5);
  return cycles;
}
...
int main(int argc,char **argv) {
  ...
  setCurrentThreadAffinityToNodeID(0);
  input = createSuperAlignedComplex(8*32*32*32*32);
  output = createSuperAlignedComplex(8*32*32*32*32);
  outputSaved = createSuperAlignedComplex(8*L*L*L*L);

//  randomInput();
//  zeroOutput();
  
  FourierTrafo4DVolumeScan(500);
//  usexFFT = true;
//  FFTbenchmark(5000, true);
