...
EvaluateObservableMultipleTimeScaleIntegration4::EvaluateObservableMultipleTimeScaleIntegration4(AnalyzerIOControl* aIOcon, StateDescriptorReader* sdr, EvaluateObservable* obsWeight, EvaluateObservable* obsDetSign, double relStart, double relEnd) : EvaluateObservable(aIOcon, sdr, "MultipleTimeScaleIntegration4", "msint4", relStart, relEnd) { 
...
int EvaluateObservableMultipleTimeScaleIntegration4::getAnalyzerResultsCount() {
...
void EvaluateObservableMultipleTimeScaleIntegration4::defineObsDependencies() { 
}


bool EvaluateObservableMultipleTimeScaleIntegration4::evaluate() {
  trajectoryLength = dataAvail[0][0]*dataAvail[0][2];
  return true;
}


LAPsystemPlot* EvaluateObservableMultipleTimeScaleIntegration4::createPlot1(int startInd, int indCount, const char* tag, const char* des) {
...
void EvaluateObservableMultipleTimeScaleIntegration4::generateLatexAndPlotsAndXML() {
  if (!MultiplePolynomFlag) {
