...
AnalyzerObservableMultipleTimeScaleIntegration3::~AnalyzerObservableMultipleTimeScaleIntegration3() {
  delete pHMCProp;
}


void AnalyzerObservableMultipleTimeScaleIntegration3::readFourierAccelerationData() {
...
int AnalyzerObservableMultipleTimeScaleIntegration3::howManyMatrixApplicationsForIntegrator(int* iter, int* Parameter_PolyDegree, int* Parameter_IntegratorType, int polyMode) {
  int howMany = 0;
  int startI = 4;
  int endI = 0;
...
double AnalyzerObservableMultipleTimeScaleIntegration3::PerformIntegration(int outerIntSteps, int outerIntType, int HiggsIntSteps, int HiggsIntType, int &Nmmdag, double &epsi, int polyMode) {
  int I;
  int Parameter_SubPolyCount = SDReader->getSubPolynomCount();
  if ((polyMode>=2) && (!MultiplePolynomFlag)) {
    printf("ERROR in AnalyzerObservableMultipleTimeScaleIntegration3::PerformIntegration: invalid value for polyMode\n");
    exit(0);
  }
...
bool AnalyzerObservableMultipleTimeScaleIntegration3::analyze(AnalyzerPhiFieldConfiguration* phiFieldConf, Complex** auxVectors) {
  if (pHMCProp==NULL) {
...
int AnalyzerObservableMultipleTimeScaleIntegration3::getNeededAuxVectorCount() {
  return 0;
}


int AnalyzerObservableMultipleTimeScaleIntegration3::getAnalyzerResultsCount() {
  if (MultiplePolynomFlag) {
    return 48;  
  } else {
    return 320;
