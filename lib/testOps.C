...
void testDistributedFourierTransformation(int opMode, bool enforceCoreTie, int threadCount) {
  printf("\nTesting Distributed Fourier-Transformation with opMode=%d, enforceCoreTie=%d, threadCount=%d...\n",opMode,enforceCoreTie,threadCount);
  randomInput();
  randomOutput();
//  int VL = fOps->getVectorLength();
...
void testDistributedMMDaggerxQuasiHermiteanNeubergerWithChi(int OpMode, bool enforceCoreTie, int threadCount, double yN, double explicitMass, bool useR, bool inFourierSpace) {
  printf("\nTesting Distributed MMdag in quasi-hermitean mode with yN = %1.2f, explicitMass=%1.2f, useR = %d, and inFourierSpace=%d...\n",yN, explicitMass, useR,inFourierSpace);  
  randomPhi(0);
  randomInput();
  randomOutput();
  int VL = fOps->getVectorLength();
...
void testMMDaggerxQuasiHermiteanNeubergerWithChi(double yN, double expMass, bool useR, bool inFourierSpace) {
  printf("\nTesting MMdag in quasi-hermitean mode with yN = %1.2f, explicit mass=%1.2f, R-Prec=%d, in FourierSpace=%d...\n",yN,expMass,useR,inFourierSpace);  
...
void testCompactMMDaggerxNeubergerWithChi(double yN, double explicitMass, bool inFourierSpace, bool precUse, double precM, double precS, bool QprecUse, double QprecMu, double QprecBeta) {
...
void testPreconditioner(double yN, double explicitMass, double m, double s) {
...
void testSolverNeubergerWithChi(double yN, double explicitMass, bool doubleM, bool QHMode, double TOL) {
  if (doubleM) {
    printf("\nTesting MMdag-Solver (Neuberger with Xi) with yN = %1.2f, explitMass=%1.2f, QHMode=%d, TOL = %1.15f ...\n",yN,explicitMass,QHMode,TOL);
  } else {
    printf("\nTesting M-Solver (Neuberger with Xi) with yN = %1.2f, explicitMass=%1.2f, QHMode=%d, TOL = %1.15f ...\n",yN,explicitMass,QHMode,TOL);  
  }
  randomPhi(0);
  randomInput();
...
void testpHMCPropagation(int OpMode, bool enforceCoreTie, bool FACC, double yN, double kappa, double lambda, bool sphMode, double sphZeta, double massSplit, double explicitMass, double current, double c6, double c8, double c10, double lam6, double lam8, double lam10, bool precUse, double precM, double precS, bool QprecUse, double QprecMu, double QprecBeta, bool RprecUse, double RprecM, double RprecF) {
  printf("\nTesting Integrators for pHMC with yN = %1.2f, kappa = %f, lambda = %1.15f, SphericalMode = %d, SphericalZeta = %1.3e, current = %1.15f, massSplit = %1.15f, explicitMass = %1.15f, c6 = %1.15f, c8 = %1.15f, c10 = %1.15f, lam6 = %1.15f, lam8 = %1.15f, lam10 = %1.15f,...\n",yN, kappa, lambda, sphMode, sphZeta,current, massSplit, explicitMass, c6, c8, c10, lam6, lam8, lam10);
  double S1, S2;
  randomPhi(0);
...
xtraSize3 = 3;
  ...
  fOps->setxFFTusage(usexFFT); 
  fOps->setAntiPeriodicBoundaryConditionsInTime(false); 
...
//  testPHMCSampleOmegaNeubergerWithChi(1.2, true, 1, 0, true, 0.5, 0.5, true, 1.0, 0.30, false);
  testpHMCPropagation(1, false, true, 1.2, 0.1, NaN, true, 0*0.23, 0.15, 0.10, 1.4, 0.0051, 0.0052, 0.0053, 0.054, 0.055, 0.056, true, 1, 0, true, 1, 0.5, true, 1.0, 1.0); 
...
//  testDistributedFourierTransformation(0,false, 1);
//  testDistributedFourierTransformation(0,false, 2);
  
//  testDistributedFourierTransformation(1,false, 3);
//  testDistributedFourierTransformation(1,false, 2);
  
//  testDistributedFourierTransformation(2,false, 1);
//  testDistributedFourierTransformation(2,false, 2);
...
//  testDistributedMMDaggerxQuasiHermiteanNeubergerWithChi(2, false, 2, 1.5, 0.10, true, true);
//  testDistributedPHMCForcesNeubergerWithChi(2, false, 1, false, 1.2, 0.15, 0.10, true, 1, 0, true, 0.5, 0.5, true, 1.0, 1.0);
...
//  testMxNeubergerWithChi(1.5, 0.15, 0.10, true, false);
//  testMxNeubergerWithChi(1.5, 0.15, 0.10, true, true);
//  testMMDaggerxNeubergerWithChi(1.5, 0.15, 0.10, true);
//  testMMDaggerxQuasiHermiteanNeubergerWithChi(1.5, 0.1, true, false);
//  testCompactMMDaggerxNeubergerWithChi(1.5, 0.1, true, true, 1, 0, true, 0.5, 0.5);

//  testPreconditioner(0.7, 0.10, 0.24, 0.03);
//  testSolverNeubergerWithChi(5, 0.10, true, true, 1E-8);
//  testMMDaggerSQTRSolverNeuberge1rWithChi(1.1, 1E-10, true, 1.0, 0, true, 0.5, 0.5, true, 1.0, 1.0, 20);
