...
AnalyzerObservableMultipleTimeScaleIntegration2::AnalyzerObservableMultipleTimeScaleIntegration2(FermionMatrixOperations* fOps, AnalyzerIOControl* aIOcon, StateDescriptorReader* SDreader) : AnalyzerObservable(fOps, aIOcon, SDreader, "MultipleTimeScaleIntegration2", "msint2") { 
...
AnalyzerObservableMultipleTimeScaleIntegration2::~AnalyzerObservableMultipleTimeScaleIntegration2() {
  delete pHMCProp;
}


void AnalyzerObservableMultipleTimeScaleIntegration2::readFourierAccelerationData() {
...
int AnalyzerObservableMultipleTimeScaleIntegration2::howManyMatrixApplicationsForIntegrator(int* iter, int* Parameter_PolyDegree, int* Parameter_IntegratorType, int polyMode) {
  int howMany = 0;
  int startI = 4;
  int endI = 0;
...
double AnalyzerObservableMultipleTimeScaleIntegration2::PerformIntegration(int outerIntSteps, int outerIntType, int HiggsIntSteps, int HiggsIntType, int &Nmmdag, double &epsi, int polyMode) {
  int I;
  int Parameter_SubPolyCount = SDReader->getSubPolynomCount();
  if ((polyMode>=2) && (!MultiplePolynomFlag)) {
    printf("ERROR in AnalyzerObservableMultipleTimeScaleIntegration2::PerformIntegration: invalid value for polyMode\n");
    exit(0);
  }
...
bool AnalyzerObservableMultipleTimeScaleIntegration2::analyze(AnalyzerPhiFieldConfiguration* phiFieldConf, Complex** auxVectors) {
  if (pHMCProp==NULL) {
...
int AnalyzerObservableMultipleTimeScaleIntegration2::getNeededAuxVectorCount() {
  return 0;
}


int AnalyzerObservableMultipleTimeScaleIntegration2::getAnalyzerResultsCount() {
  if (MultiplePolynomFlag) {
    return 32;  
  } else {
    return 320;
