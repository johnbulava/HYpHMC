...
AnalyzerObservableMultipleTimeScaleIntegrationTest::AnalyzerObservableMultipleTimeScaleIntegrationTest(FermionMatrixOperations* fOps, AnalyzerIOControl* aIOcon, StateDescriptorReader* SDreader) : AnalyzerObservable(fOps, aIOcon, SDreader, "MultipleTimeScaleIntegrationTest", "msinttest") { 
...
AnalyzerObservableMultipleTimeScaleIntegrationTest::~AnalyzerObservableMultipleTimeScaleIntegrationTest() {
  delete pHMCProp;
}

#define SETP1VALUE_Epsilon 0.006
#define SETP1VALUE_Degree 24



void AnalyzerObservableMultipleTimeScaleIntegrationTest::readFourierAccelerationData() {
...
int AnalyzerObservableMultipleTimeScaleIntegrationTest::howManyMatrixApplicationsForIntegrator(int* iter, int* Parameter_PolyDegree, int* Parameter_IntegratorType, int polyMode) {
  int howMany = 0;
  int startI = 4;
  int endI = 0;
...
double AnalyzerObservableMultipleTimeScaleIntegrationTest::PerformIntegration(int outerIntSteps, int outerIntType, int innerIntSteps, int innerIntType,int &Nmmdag, double &epsi, int polyMode) {
  int I;
  int Parameter_SubPolyCount = SDReader->getSubPolynomCount();
  if ((polyMode>=2) && (!MultiplePolynomFlag)) {
    printf("ERROR in AnalyzerObservableMultipleTimeScaleIntegrationTest::PerformIntegration: invalid value for polyMode\n");
    exit(0);
  }
...
Parameter_PolyDegree[1] = SETP1VALUE_Degree;
...
//printf("intTypeHiggs: %d\n", integrators[0]);
  ...
  if (polyMode<2) {
    ...
    integrators[Parameter_SubPolyCount] = innerIntType;
    iterations[Parameter_SubPolyCount] = innerIntSteps;
    
    
    Parameter_IntegratorType[0] = outerIntType;
    Parameter_Iterations[0] = outerIntSteps;
    Parameter_IntegratorType[1] = innerIntType;
    Parameter_Iterations[1] = innerIntSteps;
  } else {
printf("ERROR in MSintTest\n");  
exit(0);
    Parameter_Epsilon *= Parameter_Iterations[0];
    Parameter_Epsilon /= outerIntSteps;
    epsi = Parameter_Epsilon;
...
bool AnalyzerObservableMultipleTimeScaleIntegrationTest::analyze(AnalyzerPhiFieldConfiguration* phiFieldConf, Complex** auxVectors) {
  if (pHMCProp==NULL) {
...
polDeg[1] =    SETP1VALUE_Degree;
polEps[1] =    SETP1VALUE_Epsilon;
  ...
  if (MultiplePolynomFlag) {
    ...
    for (int I=1; I<=8; I++) {
      ...
      double deltaS = PerformIntegration(I, 1, 1, 2, Nmmdag, epsi, 0);
...
/*    for (int I=1; I<=8; I++) {
      ...
      int Nmmdag = 0;
      analyzerResults[count*4 + 0] = I;
      double deltaS = PerformIntegration(I, 1, 2, 1, Nmmdag, epsi, 0);
      analyzerResults[count*4 + 1] = deltaS;
      analyzerResults[count*4 + 2] = epsi;
      analyzerResults[count*4 + 3] = Nmmdag;
      count++;
    ...
    for (int I=1; I<=8; I++) {
      ...
      int Nmmdag = 0;
      analyzerResults[count*4 + 0] = I;
      double deltaS = PerformIntegration(I, 0, 3, 1, Nmmdag, epsi, 0);
      analyzerResults[count*4 + 1] = deltaS;
      analyzerResults[count*4 + 2] = epsi;
      analyzerResults[count*4 + 3] = Nmmdag;
      count++;
...
int AnalyzerObservableMultipleTimeScaleIntegrationTest::getNeededAuxVectorCount() {
  return 0;
}


int AnalyzerObservableMultipleTimeScaleIntegrationTest::getAnalyzerResultsCount() {
  if (MultiplePolynomFlag) {
    return 32;  
  } else {
    return 96;
