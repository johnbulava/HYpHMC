AnalyzerObservableMultipleTimeScaleIntegration::AnalyzerObservableMultipleTimeScaleIntegration(FermionMatrixOperations* fOps, AnalyzerIOControl* aIOcon, StateDescriptorReader* SDreader) : AnalyzerObservable(fOps, aIOcon, SDreader, "MultipleTimeScaleIntegration", "msint") { 
...
AnalyzerObservableMultipleTimeScaleIntegration::~AnalyzerObservableMultipleTimeScaleIntegration() {
  delete pHMCProp;
}


void AnalyzerObservableMultipleTimeScaleIntegration::readFourierAccelerationData() {
...
int AnalyzerObservableMultipleTimeScaleIntegration::howManyMatrixApplicationsForIntegrator(int* iter, int* Parameter_PolyDegree, int* Parameter_IntegratorType, int polyMode) {
  int howMany = 0;
  int startI = 4;
  int endI = 0;
...
double AnalyzerObservableMultipleTimeScaleIntegration::PerformIntegration(int outerIntSteps, int outerIntType, int &Nmmdag, double &epsi, int polyMode) {
  int I;
  int Parameter_SubPolyCount = SDReader->getSubPolynomCount();
  if ((polyMode>=2) && (!MultiplePolynomFlag)) {
    printf("ERROR in AnalyzerObservableMultipleTimeScaleIntegration::PerformIntegration: invalid value for polyMode\n");
    exit(0);
  }
...
bool AnalyzerObservableMultipleTimeScaleIntegration::analyze(AnalyzerPhiFieldConfiguration* phiFieldConf, Complex** auxVectors) {
  if (pHMCProp==NULL) {
  ...
  if (MultiplePolynomFlag) {
    ...
    for (int I=1; I<=8; I++) {
      double epsi = NaN;
      int Nmmdag = 0;
      double deltaS = PerformIntegration(I, 0, Nmmdag, epsi, 0);
    ...
    for (int I=1; I<=8; I++) {
      double epsi = NaN;
      int Nmmdag = 0;
      double deltaS = PerformIntegration(I, 2, Nmmdag, epsi, 1);
      analyzerResults[count*4 + 0] = I;
      analyzerResults[count*4 + 1] = deltaS;
      analyzerResults[count*4 + 2] = epsi;
      analyzerResults[count*4 + 3] = Nmmdag;
    ...
    for (int I=1; I<=8; I++) {
      double epsi = NaN;
      int Nmmdag = 0;
      double deltaS = PerformIntegration(I, 2, Nmmdag, epsi, 2);
      analyzerResults[count*4 + 0] = I;
      analyzerResults[count*4 + 1] = deltaS;
      analyzerResults[count*4 + 2] = epsi;
      analyzerResults[count*4 + 3] = Nmmdag;
    ...
    for (int I=1; I<=8; I++) {
      double epsi = NaN;
      int Nmmdag = 0;
      double deltaS = PerformIntegration(I, 2, Nmmdag, epsi, 0);
      analyzerResults[count*4 + 0] = I;
      analyzerResults[count*4 + 1] = deltaS;
      analyzerResults[count*4 + 2] = epsi;
      analyzerResults[count*4 + 3] = Nmmdag;
...
int AnalyzerObservableMultipleTimeScaleIntegration::getNeededAuxVectorCount() {
  return 0;
}


int AnalyzerObservableMultipleTimeScaleIntegration::getAnalyzerResultsCount() {
  if (MultiplePolynomFlag) {
    return 96;  
  } else {
    return 96;
